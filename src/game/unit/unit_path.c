...
static int game_unit_path_evaluate_direction(GameContext *context, GameUnit* unit, uint16_t nextX, uint16_t nextY, int8_t moveDir) {
    ...
    if (context->walkabilityGrid[nextX][nextY] == WALKABILITY_BLOCKED) score += 999;
	
    for (int i = 0; i < PATHFINDING_HISTORY_SIZE; i++) {
        if (nextX == unit->pathfindingData.lastPositionsX[i]
			&& nextY == unit->pathfindingData.lastPositionsY[i]) score += 500;
    }

    if (moveDir == unit->pathfindingData.lastDir) score -= 10;
    if (moveDir >= DIAGONALS_INDEX) score += 4;
    if (unit->pathfindingData.frustration > 10) score += (rand() % (unit->pathfindingData.frustration * 8));

    return score;
}

void game_unit_path_init(GameUnit* unit, uint16_t targetX, uint16_t targetY) {
	if(unit->pathfindingData.pathTargetX == targetX && unit->pathfindingData.pathTargetY == targetY) return;
...
static int8_t game_unit_path_get_best_direction(GameContext *context, GameUnit *unit, int targetX, int targetY) {
    ...
    for (int i = DIRECTION_NONE + 1; i <= 8; i++) {
        ...
        if (nextX < 0 || nextX >= BOARD_WIDTH || nextY < 0 || nextY >= BOARD_HEIGHT
			|| context->walkabilityGrid[nextX][nextY] == WALKABILITY_BLOCKED) continue;
